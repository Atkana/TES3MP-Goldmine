...
msgScript = require("msgChatEssentials")
...
/suicide - Kill yourself, this degrades your stats\
// - Global Out of Character Chat\
/// - Local Out of Character Chat\
/nick - Change your Character\'s Nickname\
/roll <skill/attribute> - perform a skill or attribute roll, this is based off your level in that skill\
/setcreature <creature ID> - appear to others as another creature, you cannot see yourself as the creature though\
/stuck - use this when you are stuck"
...
/ncolor <pid> <number sign|HEXCODE> - set a roleplayer's color code in local chat"
...
function OnPlayerDisconnect(pid)
    ...
    msgScript.GetScript("rpChatEssentials").OnPlayerDisconnect(pid)
...
function OnPlayerSendMessage(pid, message)
    ...
    if message:sub(1,1) == '/' then
		...
		elseif cmd[1] == "setcreature" then --and admin then
			tes3mp.SetCreatureModel(pid, tableHelper.concatenateFromIndex(cmd, 2), true)
			tes3mp.SendBaseInfo(pid)

		elseif cmd[1] == "disablecreature" and admin then
			tes3mp.SetCreatureModel(pid, "", false)
			tes3mp.SendBaseInfo(pid)

		elseif cmd[1] == "stuck" then
			stuck.ProcessStuckCommand(pid)
			
		elseif cmd[1] == "roll" then RPRolls.doRoll(pid, playerName, cmd[2])
